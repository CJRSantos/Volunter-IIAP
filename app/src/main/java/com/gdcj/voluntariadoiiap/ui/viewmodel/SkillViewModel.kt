...
sealed class SkillDetailState {
    object Idle : SkillDetailState()
    object Loading : SkillDetailState()
    data class Success(val skill: Skill) : SkillDetailState()
...
class SkillViewModel : ViewModel() {
    ...
    private val _skillDetailState = MutableStateFlow<SkillDetailState>(SkillDetailState.Idle)
    val skillDetailState = _skillDetailState.asStateFlow()
    ...
    fun fetchSkillById(id: Int) {
        viewModelScope.launch {
            _skillDetailState.value = SkillDetailState.Loading
            try {
                val response = RetrofitClient.skillService.getSkillById(id)
                if (response.isSuccessful && response.body() != null) {
                    _skillDetailState.value = SkillDetailState.Success(response.body()!!)
                } else {
                    _skillDetailState.value = SkillDetailState.Error("Error: ${response.code()}")
                }
            } catch (e: Exception) {
                _skillDetailState.value = SkillDetailState.Error(e.message ?: "Error desconocido")
            }
        }
    }
    ...
    fun updateSkill(token: String, id: Int, skill: Skill) {
        viewModelScope.launch {
            _operationState.value = OperationState.Loading
            try {
                val authToken = if (token.startsWith("Bearer ")) token else "Bearer $token"
                val response = RetrofitClient.skillService.updateSkill(authToken, id, skill)
                if (response.isSuccessful) {
                    _operationState.value = OperationState.Success("Habilidad actualizada correctamente")
                    fetchSkills()
                } else {
                    _operationState.value = OperationState.Error("Error: ${response.code()}")
                }
    ...
    fun deleteSkill(token: String, id: Int) {
        viewModelScope.launch {
            _operationState.value = OperationState.Loading
            try {
                val authToken = if (token.startsWith("Bearer ")) token else "Bearer $token"
                val response = RetrofitClient.skillService.deleteSkill(authToken, id)
                if (response.isSuccessful) {
                    _operationState.value = OperationState.Success("Habilidad eliminada correctamente")
                    fetchSkills()
                } else {
                    _operationState.value = OperationState.Error("Error: ${response.code()}")
                }
